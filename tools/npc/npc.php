...
function NPCStuff() {
	...
	while (true) {
		...
		try {
			...
			// TODO: these two could be actions as well
			if ($player->isDead()) {
			...
			foreach ($actions as $class => $options) {
				$actions = $class($options);
				if ($action->conditionsMet($player)) {
					processContainer($action->getContainer());
				}
			}
